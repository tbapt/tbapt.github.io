        ...
        if (el.attr("data-fade") === "true") {
          ...
          if (
            ...
            if (sceneDevider - pos < layerDevider) {
              opacity =
                1 +
                ((sceneDevider + pos - layerDevider) / sceneHeight / 3.0) * 5;
            } else {
              opacity =
                1 -
                ((sceneDevider - pos - layerDevider) / sceneHeight / 3.0) * 5;
    ...
    function initOwlCarousel(c) {
      ...
      c.owlCarousel({
        ...
        startPosition: c.attr("data-start-position")
          ? parseInt(c.attr("data-start-position"))
          : 0,
        smartSpeed: c.attr("data-smart-speed")
          ? parseInt(c.attr("data-smart-speed"))
          : 600,
        items: 1,
    ...
    $window.on("load resize", function () {
      ...
      if ($(".rd-navbar").hasClass("rd-navbar-sidebar")) {
        transformWidth = -(
          sidebarWidth -
          144 -
          (windowWidth - navbarInnerWidth) / 2
        ); // 144 - width of toggle-sidebar + padding of navbar link
      }
      if ($(".rd-navbar").hasClass("rd-navbar-fixed")) {
        transformWidth =
          sidebarWidth - 144 - (windowWidth - navbarInnerWidth) / 2;
      }

      if ($window.width() <= 1920) {
        $(".rd-navbar-sidebar-toggle-custom").toggle(
          function () {
            $("main").css(
              "transform",
              "translate3d(" + transformWidth + "px, 0, 0)"
            );
...
document
  .getElementById("room-type-dropdown")
  .addEventListener("change", function () {
    const selectedOption = this.options[this.selectedIndex].text;
    const roomType = selectedOption.split(" @ ")[0]; // Extract text before the "@" symbol
